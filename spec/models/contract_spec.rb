...
RSpec.describe Contract, type: :model do
  ...
  it 'requires an effective date' do
    expect(build(:contract, started_on: nil)).to_not be_valid
  end

  it 'requires an end date' do
    expect(build(:contract, ended_on: nil)).to_not be_valid
  end

  it 'requires a wage' do
    expect(build(:contract, wage: nil)).to_not be_valid
  end

  it 'has a valid bonus requirement type if present' do
    expect(build(:contract, bonus_req: bonus_req, performance_bonus: perf_bonus, bonus_req_type: Faker::Lorem.word)).to_not be_valid
  end

  it 'rejects ended_on if num_seasons is provided' do
    player = create(:player)
    contract = build(:contract, player: player, ended_on: 365.days.from_now, num_seasons: 2)
  ...
  it 'sets ended_on correctly if started during 2nd half of season and num_seasons is set' do
    player = create(:player)
    contract = build(:contract, player: player, started_on: player.team.started_on + 7.months, num_seasons: 3)
  ...
  it 'sets ended_on to end of season if started during 2nd half of season with no years and num_seasons is set to 0' do
    player = create(:player)
    contract = build(:contract, player: player, started_on: player.team.started_on + 7.months, num_seasons: 0)
  ...
  it 'provides all three fields for a performance bonus' do
    expect(build(:contract, performance_bonus: perf_bonus)).to_not be_valid
    expect(build(:contract, bonus_req: bonus_req)).to_not be_valid
    expect(build(:contract, bonus_req_type: bonus_req_type)).to_not be_valid
    expect(build(:contract, performance_bonus: perf_bonus, bonus_req: bonus_req)).to_not be_valid
    expect(build(:contract, performance_bonus: perf_bonus, bonus_req_type: bonus_req_type)).to_not be_valid
    expect(build(:contract, bonus_req: bonus_req, bonus_req_type: bonus_req_type)).to_not be_valid
    expect(create(:contract, bonus_req: bonus_req, performance_bonus: perf_bonus, bonus_req_type: bonus_req_type)).to be_valid
  end
  ...
  it 'terminates the previous contract' do
    ...
    new_contract = player.contracts.create(
      attributes_for :contract, started_on: player.currently_on
    )
