...
class MatchesCompiler
  attr_accessor :team, :results, :total, :pagination, :filters

  def initialize(team:, pagination: {}, filters: {})
    @team = team
  ...
  private
    ...
    def filter_by(attribute)
      ...
      @results = @results.where(attribute => filters[attribute])
    end
    ...
    def filter_by_team
      ...
      @results = @results.where(
        'home ILIKE :team OR away ILIKE :team',
        team: "%#{filters[:team]}%"
      )
    end
    ...
    def filter_by_result
      ...
      @results = filters[:result].map do |match_result|
        @results.joins(:team).where(match_result_condition(match_result))
      end.reduce(&:or)
    end

    def filter_by_date
      @results = @results.where(played_on: filters[:start_on]..filters[:end_on])
    end
